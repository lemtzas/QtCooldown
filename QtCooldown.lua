...
local VERSION_NOTE = "v2.1 [19 November 2014]"
...
function QtCooldown:UpdateCooldowns()
  ...
  for i = 1, #self.trackedCooldowns do
    ...
    local GAC = spl:GetAbilityCharges()
    if (GAC and GAC.nChargesMax > 0) then 
      --Handle abilities with charges
      local chargeObj = spl:GetAbilityCharges()
      if (chargeObj and chargeObj.nChargesRemaining < chargeObj.nChargesMax) then
    ...
    if (abilities[i].bIsActive) then
      for j = 1,8 do
        if (abilities[i].nId == las[j]) then
          ...
          local GAC = splObj:GetAbilityCharges()
          if (splObj:GetCooldownTime() > 0 or (GAC and GAC.nChargesMax > 0)) then
